...
class StorageManager:
    ...
    async def set(self, key: str, data: bytes) -> bool:
        """异步存储数据，带重试机制"""
        try:
            result = await self._retry_operation(self._set, key, data)
            metrics_collector.track_storage_operation('set', result)
            return result
        except Exception as e:
            metrics_collector.track_storage_operation('set', False)
            raise
            
    async def _set(self, key: str, data: bytes) -> bool:
        try:
